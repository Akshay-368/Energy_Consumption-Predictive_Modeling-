# -*- coding: utf-8 -*-
"""Copy of Energy_Consumption(Predictive_Modeling).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1i0S13UfQxc9341UAMv5CvTpLhpmuWIK8
...
# Importing Data
db = 'Predict-Energy-Consumption.csv'
df = pd.read_csv(db)
...
missing_values_index = dataset1.index.isnull().sum()
missing_values_consumption = dataset1['Consumption'].isnull().sum()
print("Missing values in index:", missing_values_index)
print("Missing values in Consumption column:", missing_values_consumption)

fig = plt.figure(figsize=(15, 10))
sns.boxplot(x=dataset1.index.hour, y='Consumption', data=dataset1)
plt.title("Energy Consumption VS Hour")
plt.xlabel("Hour")
plt.grid(True, alpha=1)
plt.show()

print("Index type:", dataset1.index.dtype)
print("Number of unique index values:", dataset1.index.nunique())

print("Length of dataset1.index.year:", len(dataset1.index.year))
print("Length of dataset1.Consumption:", len(dataset1.Consumption))
print("First few values of dataset1.index.year:", dataset1.index.year[:5])
print("First few values of dataset1.Consumption:", dataset1.Consumption[:5])

dataset1.sort_index(inplace=True)

fig = plt.figure(figsize=(15, 10))
sns.boxplot(x=dataset1.index.year, y='Consumption', data=dataset1)
...
# Assuming 'Time' is the column containing datetime.time objects
columns_to_exclude = ['Time']

# Exclude columns with datetime.time objects from resampling
numerical_columns = [col for col in dataset.columns if col not in columns_to_exclude]
newDataSet = dataset[numerical_columns].resample("D").mean()
